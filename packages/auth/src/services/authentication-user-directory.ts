...
export interface AuthenticationUserDirectory {
  ...
  recordSuccessfulLogin(userId: string, occurredAt: Date): Promise<AuthenticationAccountRecord>;
  resolveIdentity(
    identityType: AuthenticationIdentityType,
    identityValue: string,
  ): Promise<AuthenticationIdentityRecord | null>;
  setLockedUntil(userId: string, lockedUntil: Date | null): Promise<AuthenticationAccountRecord>;
}
